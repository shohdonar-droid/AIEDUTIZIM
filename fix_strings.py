import subprocess
import re

def run_tsc():
    result = subprocess.run(['npx', 'tsc', '--noEmit', 'telegram.ts'], capture_output=True, text=True)
    return result.stdout

def fix_file():
    with open('telegram.ts', 'r') as f:
        lines = f.readlines()
    
    while True:
        out = run_tsc()
        # Find the first Unterminated string literal
        match = re.search(r'telegram\.ts\((\d+),\d+\): error TS1002: Unterminated string literal\.', out)
        if not match:
            break
        
        line_num = int(match.group(1)) - 1
        
        # Combine this line with the next line, putting \n between them
        print(f"Fixing line {line_num + 1}")
        lines[line_num] = lines[line_num].rstrip('\n') + '\\n' + lines[line_num + 1].lstrip()
        del lines[line_num + 1]
        
        with open('telegram.ts', 'w') as f:
            f.writelines(lines)

fix_file()
print("Done fixing strings")
