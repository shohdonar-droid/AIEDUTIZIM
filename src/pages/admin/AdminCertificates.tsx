...
export default function AdminCertificates() {
  ...
  const [showSubjectTemplateModal, setShowSubjectTemplateModal] =
    useState(false);
  ...
  const [rewardTemplate, setRewardTemplate] = useState<CertTemplate>({
    ...
    coursePrefix:
      "ushbu sertifikat platformadan faol qatnashib kelayotganligi uchun beriladi.",
    courseSuffix: "",
  });
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const allSubs = sSnap.docs.map(
          (doc) => ({ id: doc.id, isSubjectItem: true, ...doc.data() }) as any,
        );
  ...
  const saveSubjectTemplate = async () => {
    ...
    try {
      await setDoc(
        doc(db, "settings", "certificate_subject_template"),
        subjectTemplate,
      );
  ...
  const saveRewardTemplate = async () => {
    ...
    try {
      await setDoc(
        doc(db, "settings", "certificate_reward_template"),
        rewardTemplate,
      );
  ...
  const createDoc = async () => {
    ...
    try {
      ...
      alert(
        `${type === "certificate" ? "Sertifikat" : "Shartnoma"} muvaffaqiyatli shakllantirildi!`,
      );
      setShowModal(false);
    } catch (err) {
      console.error(err);
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 mt-2 text-lg">
            Sertifikatlar va talabalar bilan shartnomalar boshqaruvi.
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2 col-span-2">
                <label className="text-xs font-black text-gray-400 uppercase tracking-widest">
                  Kurs yakunlash matni
                </label>
                <input
                  type="text"
                  value={template.completionText}
                  onChange={(e) =>
                    setTemplate({ ...template, completionText: e.target.value })
                  }
                  className="w-full px-5 py-4 bg-gray-50 rounded-2xl font-bold border-none focus:ring-2 focus:ring-blue-100"
                />
              </div>
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTemplate({ ...template, coursePrefix: e.target.value })
                  }
                  className="w-full px-5 py-4 bg-gray-50 rounded-2xl font-bold border-none focus:ring-2 focus:ring-blue-100"
                />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-black text-gray-400 uppercase tracking-widest">
                  Kursdan keyingi matn
                </label>
                <input
                  type="text"
                  value={template.courseSuffix}
                  onChange={(e) =>
                    setTemplate({ ...template, courseSuffix: e.target.value })
                  }
                  className="w-full px-5 py-4 bg-gray-50 rounded-2xl font-bold border-none focus:ring-2 focus:ring-blue-100"
                />
              </div>
            ...
            <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 italic text-sm text-blue-600">
              <p>
                <strong>Eslatma:</strong> Kurs nomi avtomat ravishda "Kurs
                oldidan matn" va "Kursdan keyingi matn" o'rtasida joylashadi.
              </p>
            </div>
          ...
          >
            <div className="flex justify-between items-center pb-4 border-b border-gray-50">
              <div className="flex items-center gap-3">
                <Settings className="h-6 w-6 text-blue-600" />
                <h3 className="text-2xl font-black text-gray-900 uppercase">
                  Mavzular uchun Sertifikat Matni
                </h3>
              </div>
              <button
                onClick={() => setShowSubjectTemplateModal(false)}
                className="p-2 text-gray-400 hover:text-red-500"
              >
                <X className="h-6 w-6" />
              </button>
            ...
            <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 italic text-sm text-blue-600">
              <p>
                <strong>Eslatma:</strong> Fan/Mavzu nomi avtomat ravishda "Kurs
                oldidan matn" va "Kursdan keyingi matn" o'rtasida joylashadi.
              </p>
            </div>
          ...
          >
            <div className="flex justify-between items-center pb-4 border-b border-gray-50">
              <div className="flex items-center gap-3">
                <Settings className="h-6 w-6 text-blue-600" />
                <h3 className="text-2xl font-black text-gray-900 uppercase">
                  Rag'batlantirish Sertifikati Matni
                </h3>
              </div>
              <button
                onClick={() => setShowRewardTemplateModal(false)}
                className="p-2 text-gray-400 hover:text-red-500"
              >
                <X className="h-6 w-6" />
              </button>
            ...
            <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 italic text-sm text-blue-600">
              <p>
                <strong>Eslatma:</strong> Ushbu matnlar rag'batlantirish
                (Reward) sertifikatlarida qo'llaniladi.
              </p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  {students.map((s, idx) => (
                    <option key={`${s.uid || "student"}_${idx}`} value={s.uid}>
                      {s.displayName}
                    </option>
                  ))}
                </select>
      ...
      {loading ? (
        ...
        <div className="py-20 text-center bg-gray-50 border-4 border-dashed border-gray-100 rounded-[40px] opacity-30">
          ...
          <p className="font-black italic">
            Hozircha berilgan sertifikatlar yo'q.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b-2 border-gray-100">
                <th className="py-4 px-6 text-sm font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">
                  №
                </th>
                <th className="py-4 px-6 text-sm font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">
                  ID
                ...
                <th className="py-4 px-6 text-sm font-black text-gray-400 uppercase tracking-widest whitespace-nowrap text-center">
                  Natija
                </th>
                <th className="py-4 px-6 text-sm font-black text-gray-400 uppercase tracking-widest whitespace-nowrap text-right">
                  Sana
                </th>
                <th className="py-4 px-6 text-sm font-black text-gray-400 uppercase tracking-widest whitespace-nowrap text-center">
                  Sertifikat
            ...
            <tbody>
              {certs.map((c, i) => {
                ...
                let avg = 100;
                if (!c.isSubjectItem) {
                  const scores = c.grades
                    ? (Object.values(c.grades) as number[])
                    : [];
                  avg =
                    scores.length > 0
                ...
                let title = c.isSubjectItem
                  ? c.testTitle
                  : c.courseId === "reward"
                    ? "Rag'batlantirish Sertifikati"
                    : courses[c.courseId] || "Kurs";
                const certId =
                  c.certificateId ||
                  c.id
                    ?.replace(/[^A-Za-z0-9]/g, "")
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-4 px-6 whitespace-nowrap">
                      <span className="font-mono font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-md text-xs border border-blue-100">
                        {c.certificateId ||
                          "YAU-" +
                            c.id
                              .replace(/[^A-Za-z0-9]/g, "")
                    ...
                    <td className="py-4 px-6">
                      <span className="font-bold text-gray-600">
                        {c.isSubjectItem ? (
                          <span className="mr-2 px-2 py-0.5 bg-purple-100 text-purple-700 rounded-md text-xs">
                            Fanlar
                          </span>
                        ) : null}
                        {c.isQuizizzItem ? (
                          <span className="mr-2 px-2 py-0.5 bg-pink-100 text-pink-700 rounded-md text-xs">
                            Quizizz
                          </span>
                        ) : null}
                        {c.courseTitle || title}
                    ...
                    <td className="py-4 px-6 text-right font-bold text-gray-500 whitespace-nowrap">
                      {(() => {
                        const ts = c.isSubjectItem
                          ? c.createdAt || c.lastAccessed
                          : c.createdAt || c.lastAccessed;
                        if (!ts) return "";
                        const dateObj = ts?.toMillis
