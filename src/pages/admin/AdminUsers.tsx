...
export default function AdminUsers() {
    ...
    const runCleanup = async () => {
      ...
      try {
        for (const login of loginsToDelete) {
          const q = query(collection(db, 'users'), where('login', '==', login), where('role', '==', 'teacher'));
          ...
          // Delete students
          const sQ = query(collection(db, 'users'), where('teacherId', '==', orgUid), where('role', '==', 'student'));
          const sSnap = await getDocs(sQ);
          for (const s of sSnap.docs) {
            await deleteDoc(doc(db, 'users', s.id));
            // delete student enrollments
            const eQ = query(collection(db, 'enrollments'), where("userId", "==", s.id));
            const eSnap = await getDocs(eQ);
            for (const d of eSnap.docs) await deleteDoc(doc(db, 'enrollments', d.id));
          }

          // Delete staff
          const stQ = query(collection(db, 'users'), where('teacherId', '==', orgUid), where('role', '==', 'staff'));
          const stSnap = await getDocs(stQ);
          for (const st of stSnap.docs) await deleteDoc(doc(db, 'users', st.id));

          // Delete courses and their tests
          const cQ = query(collection(db, 'courses'), where('teacherId', '==', orgUid));
          const cSnap = await getDocs(cQ);
          for (const c of cSnap.docs) {
            const tQ = query(collection(db, 'tests'), where('courseId', '==', c.id));
          ...
          const dQ = query(collection(db, 'departments'), where('creatorId', '==', orgUid));
          const dSnap = await getDocs(dQ);
          for (const d of dSnap.docs) await deleteDoc(doc(db, 'departments', d.id));

          const gQ = query(collection(db, 'groups'), where('creatorId', '==', orgUid));
          const gSnap = await getDocs(gQ);
          for (const g of gSnap.docs) await deleteDoc(doc(db, 'groups', g.id));

          const bQ = query(collection(db, 'billing'), where('teacherId', '==', orgUid));
  ...
  const deleteSingleUser = async (uid: string, silent = false) => {
    if (!silent && !confirm("Haqiqatan ham o'chirmoqchimisiz? Rozi bo'lsangiz, barcha ma'lumotlar o'chiriladi!")) return;
    try {
  ...
  const saveTeacher = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTeacher.uid) {
         ...
         if (!qSnap.empty) { alert("Login band!"); setTeacherSaving(false); return; }

         const email = `${editingTeacher.login.trim().toLowerCase()}@teacher.uz`;
         const response = await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${firebaseConfig.apiKey}`, {
           method: 'POST', headers: { 'Content-Type': 'application/json' },
           body: JSON.stringify({ email, password: editingTeacher.password, returnSecureToken: false })
         });
         ...
         await setDoc(doc(db, 'users', uid), {
             uid: uid, displayName: editingTeacher.displayName, phone: editingTeacher.phone || '', login: editingTeacher.login.trim(),
             password: editingTeacher.password, ball: Number(editingTeacher.ball) || 0, role: 'teacher', email: email, createdAt: serverTimestamp()
         });
      }
  ...
  const createStudentAdmin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newStudent.displayName || !newStudent.teacherId || !newStudent.departmentId || !newStudent.groupId) {
       alert("To'ldiring."); return;
    }
    ...
    try {
      ...
      await setDoc(doc(db, 'users', d.localId), {
        uid: d.localId, displayName: newStudent.displayName, phone: newStudent.phone, login: login, password: pass, email: email, role: 'student',
        teacherId: newStudent.teacherId, teacherName: org?.displayName || '', departmentId: newStudent.departmentId, 
        departmentName: departments.find(dep => dep.id === newStudent.departmentId)?.name, groupId: newStudent.groupId, 
        groupName: groups.find(g => g.id === newStudent.groupId)?.name, createdAt: serverTimestamp()
      });
  ...
  const saveStudentEdit = async (e: React.FormEvent) => {
    ...
    try {
       const deptName = departments.find(d => d.id === editingStudent.departmentId)?.name || '';
       const grpName = groups.find(g => g.id === editingStudent.groupId)?.name || '';
  ...
  if (loading) return <div className="p-10 flex items-center gap-2"><Loader2 className="animate-spin" /> Yuklanmoqda...</div>;
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h2 className="text-2xl font-black text-gray-900">Yangi Talaba Yaratish</h2>
              <button onClick={() => setShowStudentModal(false)} className="p-2 hover:bg-gray-200 rounded-xl transition-colors text-gray-500"><X /></button>
            </div>
            <form onSubmit={createStudentAdmin} className="p-8 space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700 uppercase ml-1">Tashkilotni tanlang</label>
                <select 
                  required 
                  className="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-blue-600/10 focus:border-blue-600 outline-none transition-all font-medium"
                  value={newStudent.teacherId}
                  onChange={e => setNewStudent({...newStudent, teacherId: e.target.value, departmentId: '', groupId: ''})}
                >
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  >
                    <option value="">Tanlang</option>
                    {departments.filter(d => d.creatorId === newStudent.teacherId).map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                  </select>
                </div>
                <div className="space-y-2">
                  ...
                  >
                    <option value="">Tanlang</option>
                    {groups.filter(g => g.departmentId === newStudent.departmentId).map(g => <option key={g.id} value={g.id}>{g.name}</option>)}
                  </select>
                </div>
              </div>
              <div className="pt-4 flex gap-4">
                <button type="button" onClick={() => setShowStudentModal(false)} className="flex-1 py-4 bg-gray-100 text-gray-600 rounded-2xl font-black hover:bg-gray-200">BEKOR QILISH</button>
                <button type="submit" disabled={studentCreating} className="flex-1 flex items-center justify-center gap-2 py-4 bg-blue-600 text-white rounded-2xl font-black shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all disabled:opacity-50">
                  {studentCreating ? <Loader2 className="animate-spin w-5 h-5" /> : <Save className="w-5 h-5" />} SAQLASH
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="space-y-6">
           ...
           <div className="bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden">
             <table className="w-full border-collapse">
               <thead>
                 <tr className="bg-gray-50/50 border-b border-gray-50">
                   ...
                   <th className="px-6 py-5 text-left text-xs font-black text-gray-400 uppercase tracking-widest">F.I.SH (Xodim)</th>
               ...
               <tbody className="divide-y divide-gray-50">
                 {filteredStaff.map((s, i) => (
                   <tr key={s.uid} className="hover:bg-gray-50/30 group">
                     <td className="px-6 py-4 font-bold text-gray-400">{i + 1}</td>
                     <td className="px-6 py-4 font-black uppercase text-sm">{s.displayName}</td>
                     <td className="px-6 py-4">
                     ...
                     <td className="px-6 py-4 text-sm font-mono text-gray-400 text-center">{s.password || '-'}</td>
                     <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                           <button onClick={() => deleteSingleUser(s.uid)} className="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition shadow-sm">
                             <Trash2 className="w-4 h-4" />
                           </button>
                        </div>
                     </td>
             ...
             {filteredStaff.length === 0 && (
               <div className="p-20 text-center text-gray-400 font-bold italic opacity-30">Hech qanday xodim topilmadi.</div>
             )}
           </div>
        </div>
      )}
           ...
           <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col md:flex-row gap-4 items-center">
             ...
             <div className="flex flex-wrap items-center gap-2 w-full md:w-auto flex-1">
              ...
              <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                 <button onClick={exportToCSV} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 whitespace-nowrap transition-all shadow-lg shadow-green-100">
                    <Download className="w-5 h-5" /> YUKLAB OLISH
                 </button>
                 <button onClick={() => setShowStudentModal(true)} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl font-black hover:bg-blue-700 shadow-lg shadow-blue-100">
               ...
               <table className="w-full border-collapse">
                 <thead>
                   <tr className="bg-gray-50/50 border-b border-gray-50">
                     ...
                     <th className="px-6 py-5 text-center text-xs font-black text-gray-400 uppercase">Parol</th>
                   ...
                   {filteredUsers.map((u, i) => {
                     ...
                     return (
                      <tr key={u.uid} className="hover:bg-gray-50/30 group">
                        ...
                        <td className="px-6 py-4 text-sm font-mono text-gray-400 text-center">{u.password || '-'}</td>
                        <td className="px-6 py-4 text-right">
                           <div className="flex justify-end gap-2">
                             ...
                             <button onClick={() => deleteSingleUser(u.uid)} className="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition shadow-sm">
                               <Trash2 className="w-4 h-4" />
                             </button>
                           </div>
                        </td>
            ...
            <form onSubmit={saveStudentEdit} className="p-6 space-y-6">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  >
                    <option value="">Tanlang...</option>
                    {departments.filter(d => d.creatorId === editingStudent.teacherId).map(d => (
                ...
                <div>
                  ...
                  >
                    <option value="">Tanlang...</option>
                    {groups.filter(g => g.creatorId === editingStudent.teacherId && g.departmentId === editingStudent.departmentId).map(g => (
