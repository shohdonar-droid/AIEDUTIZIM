...
export default function AdminJurnal() {
  ...
  const [allSystemUsers, setAllSystemUsers] = useState<{uid: string, name: string, dept?: string, grp?: string, org?: string}[]>([]);
  const [organizations, setOrganizations] = useState<{uid: string, name: string}[]>([]);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const grps = gSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Group));
        setDepartments(depts);
        setGroups(grps);
        
        const users: Record<string, {name: string, dept: string, grp: string, teacherId?: string, role: string}> = {};
        const systemUsers: {uid: string, name: string, dept?: string, grp?: string, org?: string}[] = [];
        ...
        uSnap.forEach(d => {
          ...
          if (userObj.role === 'teacher') {
             orgs.push({uid: d.id, name: userObj.displayName || 'Noma\'lum tashkilot'});
          }
        });
        ...
        eSnap.docs.forEach(doc => {
          ...
          if (courseInfo?.role === 'admin' && users[en.userId] && users[en.userId].role === 'student') {
            enrollments.push({
              id: doc.id,
              userId: en.userId,
        ...
        // Fetch tests
        const tSnap = await getDocs(query(collection(db, 'tests'), orderBy('createdAt', 'asc')));
        ...
        // Fetch results
        const trSnap = await getDocs(query(collection(db, 'testResults'), orderBy('createdAt', 'desc')));
        const results = trSnap.docs.map(doc => {
  ...
  if (filterOrg) filteredStudents = filteredStudents.filter(s => s.org === filterOrg);
  if (filterDept) filteredStudents = filteredStudents.filter(s => s.dept === filterDept);
  if (filterGrp) filteredStudents = filteredStudents.filter(s => s.grp === filterGrp);
  if (searchTerm) filteredStudents = filteredStudents.filter(s => s.name.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const tableData = selectedCourse
    ? filteredStudents.map((su) => {
        const en = data.find((r) => r.userId === su.uid && r.courseId === selectedCourse);
        return {
          id: su.uid,
          studentName: su.name,
  ...
  const testCols = allTests.slice(testPage * testsPerPage, (testPage + 1) * testsPerPage);
  const totalTestPages = Math.ceil(allTests.length / testsPerPage);
  ...
  const exportTests = () => {
    ...
    const exportData = matrixRows.map((row, idx) => {
      ...
      testCols.forEach((t, i) => {
        item[`Test ${testPage * testsPerPage + i + 1}`] = row.scores[t.id] !== undefined ? row.scores[t.id] : 0;
      });
      return item;
    });
  ...
  if (loading) return <div className="flex h-64 items-center justify-center border-4 border-dashed border-gray-100 rounded-3xl"><Loader2 className="h-8 w-8 animate-spin text-blue-600" /></div>;

  return (
    <div className="space-y-6">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-6">
          <div>
            <h1 className="text-4xl font-black text-gray-900 tracking-tight">Jurnal</h1>
            <p className="text-gray-500 mt-2 text-lg">Talabalar baholari va natijalari ro'yxati.</p>
          </div>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
              <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto items-center flex-1">
                 ...
                 <select 
                   value={filterOrg} 
                   onChange={e => { setFilterOrg(e.target.value); setFilterDept(''); setFilterGrp(''); }}
                 ...
                 >
                   ...
                   {organizations.map(o => <option key={o.uid} value={o.uid}>{o.name}</option>)}
                 </select>
                 <select 
                   value={filterDept} 
                 ...
                 >
                   ...
                   {groups.filter(g => g.departmentId === filterDept).map(g => <option key={g.id} value={g.id}>{g.name}</option>)}
                 </select>
              </div>
              <button onClick={exportJurnal} className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-200">
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                     <tr className="bg-gray-50/50 border-b border-gray-50">
                       <th className="px-6 py-5 text-center text-xs font-black text-gray-400 uppercase tracking-widest w-16">№</th>
                       <th className="px-5 py-5 text-left text-xs font-black text-gray-400 uppercase tracking-widest min-w-[200px]">FISH</th>
                       <th className="px-4 py-5 text-center text-xs font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">1-modul</th>
                       <th className="px-4 py-5 text-center text-xs font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">2-modul</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {tableData.map((row, idx) => {
                      const mScores = [0, 1, 2, 3, 4].map(m => row.grades?.[m] !== undefined ? row.grades[m] : null);
                      const allDone = mScores.every(s => s !== null && s >= 60);
                      
                      let rowColor = 'hover:bg-gray-50/30';
                      if (!row.isEnrolled) rowColor = 'bg-red-100/30 hover:bg-red-100/50';
                      else if (allDone) rowColor = 'bg-green-100/30 hover:bg-green-100/50';
                      else rowColor = 'bg-yellow-100/30 hover:bg-yellow-100/50';

                      return (
                         <tr key={row.id} className={`${rowColor} transition-colors`}>
                           <td className="px-6 py-6 text-center text-gray-400 font-bold">{idx + 1}</td>
                           <td className="px-5 py-6">
                             <p className="font-bold text-gray-900 line-clamp-1">{row.studentName}</p>
                           </td>
                           {[0, 1, 2, 3, 4].map(m => (
                             <td key={m} className={`px-4 py-6 text-center ${m === 4 ? 'border-l border-white/50' : ''}`}>
                               {row.grades?.[m] !== undefined ? (
                                 <span className={`px-2.5 py-1 rounded-lg text-xs font-black ${row.grades[m] >= 60 ? 'bg-white text-gray-600 shadow-sm' : 'bg-white/50 text-gray-500'}`}>
                                   {row.grades[m]}%
                                 </span>
                               ) : '--'}
                             </td>
                ...
                {tableData.length === 0 && (
                  <div className="p-10 text-center text-gray-400 font-bold opacity-50">Kurs ko'rsatkichlari topilmadi.</div>
                )}
              </div>
            </div>
          </div>
        ...
        {activeTab === 'test' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
              ...
              <button onClick={exportTests} className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-2xl font-bold hover:bg-emerald-700 transition shadow-lg shadow-emerald-200">
                <Download className="h-5 w-5" /> Yuklash
              </button>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50/50 border-b border-gray-50">
                      <th className="px-6 py-5 text-center text-xs font-black text-gray-400 uppercase tracking-widest w-16">№</th>
                      <th className="px-8 py-5 text-left text-xs font-black text-gray-400 uppercase tracking-widest min-w-[200px]">Talabalar ro'yxati</th>
                      {testCols.map((tc, idx) => (
                        <th key={tc.id} className="px-4 py-5 text-center text-xs font-black text-gray-400 uppercase tracking-widest whitespace-nowrap" title={tc.title}>
                           <span className="cursor-help hover:text-blue-600 border-b border-dashed border-gray-300">Test {testPage * testsPerPage + idx + 1}</span>
                        </th>
                      ))}
                    </tr>
                  </thead>
                      ...
                      return (
                        <tr key={idx} className="hover:bg-gray-50/30 transition-colors">
                          ...
                          {testCols.map(tc => {
                             ...
                             return (
                               <td key={tc.id} className={`px-4 py-6 text-center ${!hasScore ? 'bg-red-50' : ''}`}>
                                  {hasScore ? (
                                     <span className={`px-2.5 py-1.5 rounded-lg text-xs font-black ${row.scores[tc.id] >= 60 ? 'bg-gray-100 text-gray-600' : 'bg-gray-100 text-gray-400'}`}>
                                       {row.scores[tc.id]}%
                                     </span>
                                  ) : (
                ...
                {matrixRows.length === 0 && (
                   <div className="p-10 text-center text-gray-400 font-bold opacity-50">Test ishlagan talabalar reytingi hozircha bo'sh.</div>
                )}
              </div>
            </div>
