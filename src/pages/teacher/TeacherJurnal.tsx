...
export default function TeacherJurnal() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const dSnap = await getDocs(query(collection(db, 'departments'), where('creatorId', '==', orgUid)));
        const depts = dSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Department));
        setTeacherDepts(depts);

        const gSnap = await getDocs(query(collection(db, 'groups'), where('creatorId', '==', orgUid)));
        setTeacherGroups(gSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Group)));

        // 2. Load Organization's Students
        const sSnap = await getDocs(query(collection(db, 'users'), where('role', '==', 'student'), where('teacherId', '==', orgUid)));
        ...
        const eSnap = await getDocs(query(collection(db, 'enrollments'), where('teacherId', '==', orgUid)));
        ...
        const trSnap = await getDocs(query(collection(db, 'testResults'), where('teacherId', '==', orgUid)));
        const results: any[] = [];
  ...
  const allTeacherStudents = Object.values(studentProfiles).sort((a, b) => (a.displayName || '').localeCompare(b.displayName || ''));
  ...
  if (filterDept) displayedCourseStudents = displayedCourseStudents.filter(s => s.departmentId === filterDept);
  if (filterGrp) displayedCourseStudents = displayedCourseStudents.filter(s => s.groupId === filterGrp);
  if (searchTerm) displayedCourseStudents = displayedCourseStudents.filter(s => s.displayName?.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  if (filterDept) displayedTestStudents = displayedTestStudents.filter(s => s.departmentId === filterDept);
  if (filterGrp) displayedTestStudents = displayedTestStudents.filter(s => s.groupId === filterGrp);
  if (searchTerm) displayedTestStudents = displayedTestStudents.filter(s => s.displayName?.toLowerCase().includes(searchTerm.toLowerCase()));
     ...
     if (type === 'course') {
        ...
        exportData = displayedCourseStudents.map((student, i) => {
           ...
           return {
              ...
              "Yo'nalish": teacherDepts.find(x => x.id === student.departmentId)?.name || '-',
              "Guruh": teacherGroups.find(x => x.id === student.groupId)?.name || '-',
              "Kurs": d?.courseName || '-',
              "Modul 1": d?.grades["m1"] || "-",
              "Modul 2": d?.grades["m2"] || "-",
              "Modul 3": d?.grades["m3"] || "-",
                        ...
                        return (
                           <tr key={student.uid}>
                              ...
                              <td className="px-6 py-4 text-center">
                                 {r ? (
                                    <span className={`font-black px-6 py-2 rounded-xl text-xl ${gradeNum >= 4 ? 'bg-green-500 text-white' : gradeNum === 3 ? 'bg-yellow-500 text-white' : 'bg-red-500 text-white'}`}>
                                       {gradeNum}
                                    </span>
                                 ) : '-'}
                              </td>
