...
import { collection, getDoc, getDocs, addDoc, doc, setDoc, deleteDoc, serverTimestamp, query, orderBy, where, limit, updateDoc } from 'firebase/firestore';
...
====`;
  ...
  const loadTests = async (uid: string) => {
    ...
    const q = query(collection(db, 'tests'), where('teacherId', '==', orgId));
    try {
      ...
      handleFirestoreError(e, OperationType.LIST, 'tests');
    }
  };
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const snap = await getDocs(query(collection(db, 'courses'), where('teacherId', '==', orgUid)));
        setCourses(snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Course)));
        
        const dSnap = await getDocs(query(collection(db, 'departments'), where('teacherId', '==', orgUid)));
        setDepartments(dSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Department)));

        const gSnap = await getDocs(query(collection(db, 'groups'), where('teacherId', '==', orgUid)));
  ...
  const updateTestMetadata = async (id: string, newTitle: string, dIds: string[], gIds: string[]) => {
    if (!user) return;
    try {
      await setDoc(doc(db, 'tests', id), { title: newTitle, departmentIds: dIds || [], groupIds: gIds || [] }, { merge: true });
      await loadTests(user.uid);
      setEditingTest(null);
    } catch (err) { console.error(err); }
  };
              ...
              {editingTest?.id === test.id ? (
                <div className="flex flex-col gap-2 mb-2 w-full max-w-xl">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <button onClick={() => updateTestMetadata(test.id, editingTest.title, editingTest.departmentIds, editingTest.groupIds)} className="bg-blue-600 text-white p-2 rounded-lg"><Save className="w-4 h-4" /></button>
                ...
                <h4 className="text-lg font-bold text-gray-900 flex flex-wrap items-center gap-2 mb-2">
                  ...
                  {test.title}
                  <button onClick={() => setEditingTest({ id: test.id, title: test.title, departmentIds: test.departmentIds || [], groupIds: test.groupIds || [] })} className="text-gray-400 hover:text-blue-600"><Edit className="w-4 h-4" /></button>
      ...
      {activeTab === 'ai' ? (
        <div className="space-y-10">
          ...
          <div className="bg-white rounded-3xl p-10 border border-gray-100 shadow-sm space-y-8">
             <div className="flex items-center gap-4 mb-4">
               ...
               <div>
                 <h3 className="text-2xl font-black text-gray-900">Mavzuli testlar</h3>
                 <p className="text-gray-500 text-sm mt-1">Siz yaratgan barcha mavzuli testlar ro'yxati.</p>
               </div>
             </div>
        ...
        <div className="bg-white rounded-3xl p-10 border border-gray-100 shadow-sm space-y-8">
           <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h3 className="text-2xl font-black text-gray-900">Imtihonlarim</h3>
                <p className="text-gray-500 text-sm mt-1">Siz rejalashtirgan barcha imtihonlar ro'yxati.</p>
              </div>
           </div>
